/*
  ...
  LED   D5  WS2812B : per-step blink — white=step, red=accent, blue=slide
*/
...
static Adafruit_NeoPixel led(1, MOD2_PIN_LED, NEO_GRB + NEO_KHZ800);
...
static void triggerStep(int idx) {
  ...
  // LED feedback
  if (s.gate) {
    uint32_t col = isAcc      ? led.Color(255, 40, 20)      // accent = red
                 : s.slide    ? led.Color(0, 80, 255)       // slide  = blue
                              : led.Color(255, 255, 255);   // step   = white
    if (isAcc) ledBlink(s.slide ? 95 : 55, col);
    else       ledBlink(s.slide ? 60 : 20, col);
  }
}
...
static void handleButton() {
  ...
  if (down && !wasDown) {
    ...
    if (held > 520) {
      ...
      clickCount = 0;
      ledBlink(120, led.Color(255, 255, 255));
      return;
    }
    ...
    if (clickCount == 2) {
      ...
      ledBlink(90, led.Color(255, 255, 255));
      clickCount = 0;
    }
  }
  ...
  if (clickCount == 1 && (millis() - lastClickMs) > 380) {
    ...
    ledBlink(70, led.Color(255, 255, 255));
    clickCount = 0;
  }
}
